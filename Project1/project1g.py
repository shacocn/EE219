...
if __name__ == "__main__":
    ...
    GaussianNBpredicted_svd = classifier.predict(svd_test)
    prediction_prob_lsi = classifier.predict_proba(svd_test)

  #  GaussianNBpredicted_nmf = classifier.predict(nmf_test)
  #  prediction_prob_nmf = classifier.predict_proba(nmf_test)

    print ("Statistics of Naive Bayes classifiers:")

    #lsi
    project1e.print_stats (test_data.target, GaussianNBpredicted_svd, 'LSI')
    project1e.plot_roc(test_data.target, prediction_prob_lsi[:,1], 'Naive_Bayes - LSI')

    #nmf
  #  project1e.print_stats (test_data.target, GaussianNBpredicted_nmf, 'NMF')
  #  project1e.plot_roc(test_data.target, prediction_prob_nmf[:,1], 'Naive_Bayes - NMF')
